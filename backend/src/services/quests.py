...
def get_or_create_progress(user_id: int, quest_id: int, db: Session):
    ...
    if not progress:
        progress = UserQuestProgress(
            user_id=user_id, quest_id=quest_id, progress=0, completed=False, date=today
        )
        db.add(progress)
        db.commit()
        db.refresh(progress)
...
def generate_daily_quests(
    db: Session, user_id: int, num_quests: int = DAILY_QUEST_COUNT
):
    today = date.today()
    ...
    quests = (
        ...
        .filter(
            Exercise.category == category, Exercise.difficulty == user.quest_difficulty
        )
        .all()
    )
...
def get_daily_quests(db: Session, user_id: int):
    ...
    existing = (
        db.query(UserQuestProgress)
        .filter(UserQuestProgress.user_id == user_id, UserQuestProgress.date == today)
        .all()
    )
    ...
    if existing:
        logging.info(
            f"User {user_id} already has daily quests for today, returning existing quests"
        )
...
def complete_quest(user_id: int, quest: Quest, db: Session):
    logging.info(
        f"User {user_id} completed quest '{quest.name}', awarding {quest.xp_reward} XP and item id={quest.item_reward}"
    )
    ...
    logging.info(
        f"Rewards for quest completion processed successfully for user {user_id}"
    )
