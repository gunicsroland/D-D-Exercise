...
from sqlalchemy import Integer, Boolean, String, ForeignKey, Enum, TIMESTAMP, func, Table, DateTime, Column
...
class User(Base):
    ...
    quest_difficulty: Mapped[ExerciseDifficulty] = mapped_column(Enum(ExerciseDifficulty), default=ExerciseDifficulty.VeryEasy)
...
class Character(Base):
    ...
    abilities: Mapped[list["CharacterAbility"]] = relationship("CharacterAbility", back_populates="character", cascade="all, delete")
...
class CharacterAbility(Base):
    ...
    character: Mapped["Character"] = relationship("Character", back_populates="abilities")      
...
class Quest(Base):
    ...
    xp_reward: Mapped[int] = mapped_column(Integer)
    item_reward: Mapped[int | None] = mapped_column(ForeignKey("items.id"), nullable=True)
...
class Item(Base):
    ...
    effects: Mapped[list["ItemEffect"]] = relationship("ItemEffect", secondary=item_effect_link, back_populates="items")
...
class ItemEffect(Base):
    ...
    items: Mapped[list["Item"]] = relationship("Item", secondary=item_effect_link, back_populates="effects")
...
class AdventureSession(Base):
    ...
    user: Mapped["User"] = relationship("User")
    messages: Mapped[list["AdventureMessage"]] = relationship("AdventureMessage", back_populates="session", cascade="all, delete-orphan")

class AdventureMessage(Base):
    __tablename__ = "adventure_messages"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    session_id: Mapped[int] = mapped_column(ForeignKey("adventure_sessions.id"), nullable=False)
