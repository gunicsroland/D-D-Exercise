...
def get_exercises_by_category(
    category: ExerciseCategory, db: Session = Depends(get_db)
):
...
def get_exercises_by_difficulty(
    difficulty: ExerciseDifficulty, db: Session = Depends(get_db)
):
...
def get_exercises_by_category_and_difficulty(
    ...
    logging.info(
        f"Fetching exercises in category '{category}' with difficulty '{difficulty}'"
    )
    return (
        db.query(Exercise)
        .filter(Exercise.category == category, Exercise.difficulty == difficulty)
        .all()
    )
...
def add_exercise(
    ...
    logging.info(
        f"Exercise '{exercise.name}' added successfully with id={new_exercise.id}"
    )
