...
app = APIRouter(prefix="/quests", tags=["quests"])
...
def create_quest(
    ...
    logging.info(
        f"Admin user {admin_user.id} is creating a new quest with name '{quest.name}'"
    )
    ...
    logging.info(
        f"Quest '{new_quest.name}' created successfully with id={new_quest.id}"
    )
    return new_quest
...
def delete_quest(
    ...
    logging.info(
        f"Admin user {admin_user.id} is attempting to delete quest with id={quest_id}"
    )
    quest = db.query(Quest).filter(Quest.id == quest_id).first()
    if not quest:
...
def update_quest(
    ...
    logging.info(
        f"Admin user {admin_user.id} is attempting to update quest with id={quest_id}"
    )
    quest = db.query(Quest).filter(Quest.id == quest_id).first()
    if not quest:
