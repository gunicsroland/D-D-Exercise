...
export default function ExerciseScreen() {
  ...
  const [questDifficulty, setLocalQuestDifficulty] =
    useState<ExerciseDifficulty>("very_easy");
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      setExerciseError(
        "Nem sikerült a edzéseket elérni, ellenőrizd a kapcsolatod!",
      );
    }
    ...
    try {
      ...
      setProgressError(
        "Nem sikerült a haladásodat elérni, ellenőrizd a kapcsolatod!",
      );
    }
  };
  ...
  const updateQuestDifficulty = async (
    token: string,
      ...
      <>
        <ScrollView style={exercise_styles.screen}>
          <View style={exercise_styles.section}>
            <Text style={exercise_styles.title}>Napi küldetések</Text>

            <Text style={exercise_styles.subtitle}>Nehézség:</Text>
            ...
            <View style={exercise_styles.filterRow}>
              {Object.keys(DIFFICULTY_ORDER).map((diff) => (
                <TouchableOpacity
                  key={diff}
                  onPress={() =>
                    updateQuestDifficulty(token, diff as ExerciseDifficulty)
                  }
                  style={[
            ...
            {questError ? (
              <Text style={exercise_styles.error}>{questError}</Text>
            ) : null}
            {progressError ? (
              <Text style={exercise_styles.error}>{progressError}</Text>
            ) : null}
          ...
          {ExerciseError ? (
            <Text style={exercise_styles.error}>{ExerciseError}</Text>
          ) : null}
